...
If a tool is unavailable, use this backup specification: include YAML front matter with `filename`, `document: presentation`, `title`, `theme`, and `ratio`; separate slides with top-level `---`; use one `# H1` title per slide; place at most one supported `<!--slide: name-->` directive immediately after a separator and before the title; use concise `<!--notes: ...-->` comments on content slides; and do not invent image URLs or local paths. Supported directives are `cover`, `default`, `two-columns`, `image-right`, `divider`, `section-divider`, `table`, `table-legend`, and `thanks`.
