...
export function createPresentationPrompt(request: GenerationPromptRequest): string {
  ...
  return [
    ...
    'Every slide must start with one # H1 slide title, optionally preceded by exactly one <!--slide: name--> directive.',
    ...
    '- Slide directives must appear immediately after the slide separator and before the # H1 title.',
...
export function createPresentationRepairPrompt(markdown: string, errors: string[]): string {
  return [
    ...
    '- slide directives appear only at the start of a slide',
...
export function presentationImageInstruction(): string {
  return [
    'Image handling:',
    ...
    '- For visual subjects without a provided asset, add an image suggestion in speaker notes or a blockquote, for example: > Image suggestion: a classical depiction of Athena with owl and shield.',
