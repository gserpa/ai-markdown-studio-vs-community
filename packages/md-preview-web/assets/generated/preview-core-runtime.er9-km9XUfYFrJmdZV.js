(async () => {
  ...
  window.__mfoPreviewModeRuntime = {
    startDocument() {
    ...
    startPresentation() {
      if (modeStarted) return;
      modeStarted = true;
      wrapPresentationTablesForScroll();
  ...
  const hasMermaidBlocks = Boolean(document.querySelector('.mermaid, .mermaid-rendered[data-mermaid-source]'));
  let mermaid = window.mermaid;
  if (hasMermaidBlocks && !mermaid && mermaidModuleUri) {
    try {
...
function wrapTablesForScroll({ forceWrap = false } = {}) {
  const defaultLayout = forceWrap || document.body.dataset.documentTableLayout === 'wrap' ? 'wrap' : 'wide';
  ...
  for (const table of tables) {
    ...
    const isPresentationTable = Boolean(table.closest('.presentation-slide-body'));
    if (isPresentationTable && !forceWrap) {
      continue;
    }

    if (table.parentElement?.classList.contains('table-scroll-wrapper')) {
      if (forceWrap) {
        table.parentElement.dataset.tableLayout = 'wrap';
      }
    ...
    wrapper.dataset.tableControls = isPresentationTable
      ? 'Table layout controls for presentation slides'
      : 'Wide table controls for document preview';
...
function wrapPresentationTablesForScroll() {
  for (const table of document.querySelectorAll('.presentation-slide-body table')) {
    if (!(table instanceof HTMLTableElement) || table.parentElement?.classList.contains('table-scroll-wrapper')) {
      continue;
    }

    const wrapper = document.createElement('div');
    wrapper.className = 'table-scroll-wrapper presentation-table-scroll';
    wrapper.dataset.tableLayout = 'wrap';
    wrapper.dataset.tableControls = 'Table layout controls for presentation slides';
    table.parentElement?.insertBefore(wrapper, table);
    wrapper.appendChild(table);
    enhanceTableScrollWrapper(wrapper, 'wrap');
  }
}
...
function enhanceTableScrollWrapper(wrapper, defaultLayout) {
  ...
  toggleButton.addEventListener('click', (event) => {
    ...
    updateToggle();
    if (wrapper.closest('.presentation-slide-body')) {
      window.requestAnimationFrame(() => {
        updateTableOverflowState(wrapper);
...
function updateTableControlGeometry(wrapper) {
  ...
  const headerHeight = Math.max(34, header instanceof HTMLElement ? header.offsetHeight : 40);
